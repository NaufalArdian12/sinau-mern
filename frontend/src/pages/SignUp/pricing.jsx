...
export default function Pricing({ data }) {
    const { isPending, mutateAsync } = useMutation({ mutationFn: () => postSignUp(data) });
    ...
    const submitData = async () => {
        try {
            ...
            window.location.replace(result.data.midtrans_payment_url);
        } catch (error) {
    ...
    return (
        <div className="relative flex flex-col flex-1 p-[10px]">
            <div className="absolute w-[calc(100%-20px)] min-h-[calc(100vh-20px)] h-[calc(100%)] bg-[#060A23] -z-10 rounded-[20px]">
            ...
            <div className="grid grid-cols-2 gap-[30px] max-w-[840px] mx-auto mt-[60px]">
                ...
                <div className="card flex flex-col h-fit rounded-[20px] border border-[#262A56] p-[30px] gap-[30px] bg-[#080A2A]">
                    ...
                    <hr className="border-[#262A56]" />
                    <div className="flex flex-col gap-3">
                        <button type='button' onClick={submitData} disabled={isPending} className="flex items-center justify-center gap-3 w-full rounded-full border p-[14px_20px] transition-all duration-300 hover:bg-[#662FFF] hover:border-[#8661EE] hover:shadow-[-10px_-6px_10px_0_#7F33FF_inset] bg-[#662FFF] border-[#8661EE] shadow-[-10px_-6px_10px_0_#7F33FF_inset]">
