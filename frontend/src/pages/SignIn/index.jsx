...
export default function SignInPage() {
    ...
    }); // TODO: implement mutation for sign in
    ...
    const onSubmit = async(data) => {
        try {
            ...
            secureLocalStorage.setItem(STORAGE_KEY, response.data.token);
  ...
  return (
     <div className="relative flex flex-col flex-1 p-[10px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-[400px] h-fit rounded-[20px] border border-[#262A56] p-[30px] gap-[30px] bg-[#080A2A] m-auto mt-20">
            <div>
            ...
            <hr className="border-[#262A56]" />
            <button disabled={isLoading} type="submit" className="hover:cursor-pointer w-full rounded-full border p-[14px_20px] text-center font-semibold text-white bg-[#662FFF] border-[#8661EE] shadow-[-10px_-6px_10px_0_#7F33FF_inset]">
                Sign In to Manage
            </button>
        </form>
    </div>
